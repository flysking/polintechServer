    ...
    useLayoutEffect(() => {
      ...
      useEffect(() => {
        const fetchBoards = async () => {
            if(category==='전체게시판'){
                try {
                    const response = await fetch(`https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/BoardListPopularByCate/${category}`);
                ...
                try {
                    const response = await fetch(`https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/BoardListPopularByCate/학사일정`);
                    const json = await response.json();
        ...
        {noCategory ? (
            ...
            <TouchableOpacity  onPress={() => navigation.navigate('BoardDetail', { boardId: item.board_id })}>
                <View styles={{flexDirection:'row'}}>
                    <Text style={{color:'black', fontSize:20}}>[{item.board_subcategory}] {item.board_title} [{item.like_count}]</Text>
                </View>
                <View styles={{flexDirection:'row'}}>
