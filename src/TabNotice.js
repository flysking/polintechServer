    ...
    useLayoutEffect(() => {
      ...
      useEffect(() => {
        const fetchBoards = async () => {
            if(category==='전체게시판'){
                try {
                    const response = await fetch(`https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/BoardListNoticeByCate/${category}`);
                ...
                try {
                    const response = await fetch(`https://port-0-polintechservercode-ac2nlkzlq8aw.sel4.cloudtype.app/BoardListNoticeByCate/학사일정`);
                    const json = await response.json();
